...
func (f *GitLabDeviceFlow) RequestCode(ctx context.Context) (DeviceCodeResponse, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		if decErr := json.NewDecoder(resp.Body).Decode(&errBody); decErr == nil && errBody.Error != "" {
			return DeviceCodeResponse{}, fmt.Errorf("GitLab device code request failed (HTTP %d): %s — %s", resp.StatusCode, errBody.Error, errBody.ErrorDescription)
		}
		return DeviceCodeResponse{}, fmt.Errorf("GitLab device code request failed with HTTP %d", resp.StatusCode)
	}
